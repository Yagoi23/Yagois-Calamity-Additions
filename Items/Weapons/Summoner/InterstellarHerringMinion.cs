...
{
    public class InterstellarHerringMinion : HerringMinion
    {
		...
		public override void AI()
		{
			...
			bool num3 = ((ModProjectile)this).Projectile.type == ModContent.ProjectileType<InterstellarHerringMinion>();
			val.AddBuff(ModContent.BuffType<InterstellarHerringBuff>(), 3600, true, false);
			//YagoisCalmityAdditionsPlayer modPlayer = player.GetModPlayer<YagoisCalmityAdditionsPlayer>();
			if (num3)
			{
				if (val.dead)
				{
					modPlayer.InterstellarHerring = false;
				}
		...
		public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			...
			target.AddBuff(ModContent.BuffType<HolyInferno>(), 600);
			...
			target.AddBuff(ModContent.BuffType<BanishingFire>(), 600);
			target.AddBuff(ModContent.BuffType<DemonFlames>(), 600);
			target.AddBuff(ModContent.BuffType<Nightwither>(), 600);
			target.AddBuff(ModContent.BuffType<SagePoison>(), 600);
			target.AddBuff(ModContent.BuffType<ArmorCrunch>(), 600);
			target.AddBuff(ModContent.BuffType<WitherDebuff>(), 600);
